...
class PolygonsTestCase(unittest.TestCase):
    ...
    def test_wrong_shape(self):
        points = [np.zeros([4, 2]), np.zeros([2, 2])]
    ...
    def test_has_field(self):
        points = [np.zeros([4, 2]), np.zeros([4, 2]), np.zeros([4, 2])]
        polygons = Polygons(points)
    ...
    def test_get_data(self):
        points = [np.zeros([4, 2])]
        polygons = Polygons(points)
        points_modified = polygons.get_data()
        points_modified[0][0][0] = 1
        np.testing.assert_equal(points_modified, np.array([[[1, 0], [0, 0], [0, 0], [0, 0]]]))
        points_original = polygons.get_data()
        np.testing.assert_equal(points_original, np.array([[[0, 0], [0, 0], [0, 0], [0, 0]]]))

    def test_set_data(self):
        points = [np.zeros([4, 2])]
        polygons = Polygons(points)
        polygons.set_data([np.ones([4, 2])])
        points = polygons.get_data()
        np.testing.assert_equal(points, np.array([[[1, 1], [1, 1], [1, 1], [1, 1]]]))
    ...
    def test_scale_and_pad(self):
        ...
        np.testing.assert_equal(polygons.get_data(), target_points)
    ...
    def test_scale_and_pad_0(self):
        ...
        polygons = Polygons(points)
        polygons.scale_and_pad([2, 2], [1, 0])
        np.testing.assert_equal(polygons.get_data(), [])
